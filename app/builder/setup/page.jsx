...
export default function Setup() {
  ...
  return (
    <div>
      ...
      <div className="card">
        <div className="cardHeader">
          <div>
            <h1>Builder Setup</h1>
            <div className="hint">
              Configure required trades and the bid submission template per trade. Manage documents visible to all registered subcontractors.
              <br />
              Community code (share with all subs): <b>{cfg.communityCode}</b>
            </div>
          </div>
          <div className="row">
        ...
        <h2>Bid templates</h2>
        <div className="hint">
          Choose <b>Lumpsum</b> (vendor enters one amount per plan) or <b>Line items</b> (builder defines items + UOM + takeoffs per plan; vendor enters unit price).
        </div>
        ...
        <div className="cardHeader">
          <div>
            <h2>Documents</h2>
            <div className="hint">Only Builder can add/edit/remove. Vendors can view once registered.</div>
          </div>
        ...
        <div className="tablewrap" style={{ marginTop: 10 }}>
          <table>
            ...
            <tbody>
              {docs.length === 0 ? (
                <tr><td colSpan={4} className="small">No documents posted yet.</td></tr>
              ) : docs.map((d) => (
                <tr key={d.id}>
                  <td>{d.title}</td>
                  ...
                  <td>
                    <div className="row">
                      <button className="btn" onClick={() => editDoc(d)}>Edit</button>
                      <button className="btn bad" onClick={() => deleteDoc(d)}>Remove</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Modal
        open={!!openTrade}
        title={"Edit Template — " + (tradeCatalog.find((t) => t.id === openTrade)?.name || "")}
...
function TemplateEditor({ tradeId, cfg, tpl, onSetMode, onAddItem, onUpdateItem, onRemoveItem }) {
      ...
      {tpl.mode === "lumpsum" ? (
        ...
        <div style={{ marginTop: 12 }}>
          <div className="cardHeader">
            <div>
              <h2>Line items</h2>
              <div className="hint">Define items + UOM + takeoffs per plan. Vendors enter unit pricing.</div>
            </div>
            ...
            <table>
              <thead>
                <tr>
                  <th>Item</th>
                  <th>UOM</th>
                  {plans.map((p) => <th key={p.id}>Qty ({p.name})</th>)}
                  <th></th>
                </tr>
              </thead>
              <tbody>
                {(tpl.items || []).length === 0 ? (
                  <tr><td colSpan={3 + plans.length} className="small">No items yet. Add an item.</td></tr>
                ) : (tpl.items || []).map((it) => (
                  <tr key={it.id}>
                    ...
                    <td><input className="input" style={{ minWidth: 140 }} value={it.uom} onChange={(e) => onUpdateItem(tradeId, it.id, { uom: e.target.value })} placeholder="UOM (SF, LF, EA)" /></td>
                    {plans.map((p) => (
                      <td key={p.id}>
                        <input
                          className="input"
          ...
          <div className="hint" style={{ marginTop: 10 }}>Tip: set Qty=1 if you want a line to behave like a lumpsum line.</div>
        </div>
      )}
    </div>
  );
